...
type Props = { legalDocuments: RequiredLegalDocument[] }

function PanierContent({ legalDocuments }: Props) {
  ...
  // Agreement to the association's own legal documents. Collected here, on our page, because
  // the STRIPE rail hands the buyer straight over to Stripe's hosted payment page — there is
  // no later step of ours to ask on. The revision ids are what gets sent, so wording rewritten
  // after this page loaded is refused server-side rather than silently accepted.
  ...
  const legalReady = legalDocuments.length === 0 || legalAccepted
  const canSubmit  = firstName.trim() && lastName.trim() && emailValid && items.length > 0 && deliveryReady && legalReady
...
export function BoutiquePanierView({ legalDocuments }: Props) {
  return (
    <Suspense fallback={null}>
      <PanierContent legalDocuments={legalDocuments} />
    </Suspense>
  )
}
