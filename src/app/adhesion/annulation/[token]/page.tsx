...
export default function CancelCotisationSubscriptionPage() {
  ...
  useEffect(() => {
    fetch(`/api/public/cotisation-subscriptions/${token}`)
  ...
  async function handleConfirm() {
    setSubmitting(true)
    try {
      const res = await fetch(`/api/public/cotisation-subscriptions/${token}`, { method: "POST" })
    ...
    <div className="min-h-screen bg-gradient-to-b from-primary/5 to-background flex items-start justify-center py-12 px-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Adhésion récurrente</h1>
        ...
        <div className="rounded-lg border bg-card p-6 space-y-4 text-center">
          {done ? (
            <p className="font-medium">Adhésion récurrente arrêtée.</p>
          ) : info.cancelled ? (
            <p className="text-sm text-muted-foreground">Cette adhésion récurrente est déjà arrêtée.</p>
          ) : (
            <>
              <p className="text-sm text-muted-foreground">
                {info.firstName} {info.lastName} — {Number(info.amount).toLocaleString("fr-FR", { style: "currency", currency: "EUR" })} / an
              </p>
              <p className="text-sm">Confirmez que vous souhaitez arrêter le prélèvement automatique de votre cotisation.</p>
              <Button
                variant="destructive"
                loading={submitting}
