...
export const DELETE = withAdminAuth<{ id: string }>(async (_req, ctx, { id }) => {
  ...
  if (form._count.dons > 0 || form._count.subscriptions > 0) {
    return NextResponse.json(
      { error: "Impossible de supprimer : ce formulaire a déjà reçu des dons. Archivez-le plutôt." },
      { status: 409 },
    )
  }

  await prisma.donationForm.delete({ where: { id } })

  await writeActivityLog({
    associationId: ctx.associationId,
    actorId:       ctx.userId,
    action:        "DONATION_FORM_DELETED",
    entity:        "DonationForm",
    entityId:      id,
    label:         form.title,
  })
