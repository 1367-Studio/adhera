...
export const DELETE = withAdminAuth<{ id: string }>(async (_req, ctx, { id }) => {
  ...
  const cancelled = await cancelActiveCotisationSubscriptionForMembre(id, {
    actorId: userId,
    label:   `${membre.firstName} ${membre.lastName}`,
  })
  if (!cancelled) {
    return NextResponse.json({ error: "Aucune adhésion récurrente active à arrêter, ou l'arrêt a échoué." }, { status: 409 })
  }
