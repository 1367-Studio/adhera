...
export async function GET(
  ...
  // Keyed by token, not IP — see billet-info; more generous here since mail clients and
  // proxies (Gmail) may refetch the image, and it stays cheap because invalid tokens
  // never reach the QR encoder.
  if (!(await rateLimit(`billet-qr:${token}`, 60, 10 * 60_000))) {
