...
// Sent to the buyer after an admin refunds/cancels a paid boutique order, in full or in
// part — see /api/boutique/commandes/[id]/refund. `items` lists only what was actually
// refunded (quantities returned), not the order's remaining contents.
export function boutiqueRefundEmail(p: {
  firstName:       string
  email:           string
  associationName: string
  ...
  const amountStr = (p.refundedAmount / 100).toLocaleString("fr-FR", { style: "currency", currency: "EUR" })
  ...
  const intro = p.fullyCancelled
    ? `votre commande auprès de <strong>${p.associationName}</strong> a été annulée et remboursée.`
    : `une partie de votre commande auprès de <strong>${p.associationName}</strong> a été remboursée.`

  const content = `
    <h2 style="margin:0 0 8px;font-size:20px;font-weight:700;">${p.fullyCancelled ? "Commande annulée" : "Remboursement partiel"}</h2>
    <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#3f3f46;">
      Bonjour ${p.firstName},<br>${intro}
    </p>
    <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;width:100%;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px 20px;box-sizing:border-box;">
      <thead>
        <tr>
          <th style="text-align:left;font-size:12px;color:#6b7280;font-weight:600;padding-bottom:8px;">Article remboursé</th>
          <th style="text-align:center;font-size:12px;color:#6b7280;font-weight:600;padding-bottom:8px;">Qté</th>
          <th style="text-align:right;font-size:12px;color:#6b7280;font-weight:600;padding-bottom:8px;">Prix</th>
        </tr>
      </thead>
      ...
      <tfoot>
        <tr>
          <td colspan="2" style="padding-top:12px;font-size:14px;font-weight:700;">Total remboursé</td>
          <td style="padding-top:12px;font-size:16px;font-weight:700;text-align:right;">${amountStr}</td>
        </tr>
      </tfoot>
    </table>
    <p style="margin:0;font-size:13px;color:#71717a;">Si le paiement a été réglé par carte, comptez quelques jours ouvrés pour que le remboursement apparaisse sur votre compte.</p>`
