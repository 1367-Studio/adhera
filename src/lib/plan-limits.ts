...
// look (no logo) instead of an error.
export function resolveDocumentBranding(
  association: { plan: AssociationPlan; customBrandingEnabled: boolean | null; logoUrl: string | null },
): { logoUrl: string | null } {
  if (!canUseCustomBranding(association)) return { logoUrl: null }
  return { logoUrl: association.logoUrl }
}
